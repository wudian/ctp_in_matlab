...
DBwriter::DBwriter(){
	...
	if (!db.open()){
		QMessageBox::information(0, QStringLiteral("数据库错误"), QStringLiteral("无法打开数据库"));
		abort();
	}
	//获得数据库中的表名
	QSqlQuery query(db);
...
void DBwriter::processFile(QString filename){
	...
	if (!file.open(QIODevice::ReadOnly | QIODevice::Text)){
		QMessageBox::information(0, QStringLiteral("错误"), QStringLiteral("打开") + filename + QStringLiteral("时发生错误"));
		abort();
	}
	//获得纯粹的文件名
	...
	if (tables.find(tableName) == tables.end()){
		QMessageBox::information(0, QStringLiteral("错误"), QStringLiteral("数据库中没有") + tableName + QStringLiteral("表"));
		abort();
	}
	QString tableName_day = tableName + "_day";
	if (tables.find(tableName_day) == tables.end()){
		QMessageBox::information(0, QStringLiteral("错误"), QStringLiteral("数据库中没有") + tableName_day + QStringLiteral("表"));
		abort();
	}
	//检查通过开始写入数据
	QTextStream in(&file);
	...
	if (!logFile.open(QIODevice::ReadWrite | QIODevice::Text | QIODevice::Append)){
		QMessageBox::information(0, QStringLiteral("错误"), QStringLiteral("打开persistence.log时发生错误"));
		abort();
	}
