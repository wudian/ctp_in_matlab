...
s = 10; % Select Top_N for each tick, store'em in 
rsb = 30; % Threthhold
...
SSI = zeros(s,nt); % Selected Security Index
sbr_topN = zeros(s,1);
% Define UserData Struct for Timer Obj
scanTimes = 0; 
UD = struct('filename',filename,'si',si,'t0',t0,'DP',DP,'DSV',DSV,'DBV',DBV,'SBR',SBR,'SSI',SSI,'sbr_topN',sbr_topN,'rsb',rsb,'scanTimes',scanTimes);
