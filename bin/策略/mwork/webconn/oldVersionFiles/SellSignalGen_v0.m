function [SelMat, TypeMat] = SellSignalGen(...
...
%   OUTPUT
%   SelMat : Sell signals
%   TypeMat: Sell signal types. 1, exit after nD days; 2, fixStop; 
...
%           SelMat = SellSignalGen(kData,SigMat,4, 20, 0.05,0.1,0.7 );
% *********************************************************************
...
if ns>0
    ...
    linearInd = sub2ind(size(SigMat),sig_subi, sig_subj);
    SelMat = false(size(SigMat));
    SelMat(linearInd) = true;
