...
for i = 1 : n_sample-ct 
    if sampled_day_data(i,3) <  sampled_day_data(i+1,3) %% ct连增
        ...
        if l1 && sampled_day_data(i+ct,2) == max(sampled_day_data(1:i+ct,2)) ...
             ...
             && sampled_day_data(i+ct,2) - min(sampled_day_data(i:i+ct,3)) >= amp_down              %% 如果l1 == 1说明找到了一个
          k = k + 1; 
          signal_in_sec(k) = i + ct;  %% ct连减的最后一个sec的序号
          tmp(k) = 1;                 %% 连增买入 +1
        end
    elseif sampled_day_data(i,4) >  sampled_day_data(i+1,4) %% ct增
        l1 = 1;
        for j = i:i+ct-1
            if sampled_day_data(j,4) <= sampled_day_data(j+1,4) %% 存在反趋势，则此ct连增不成立
                l1 =0; break;
            end
        end   
        if l1 && sampled_day_data(i+ct,2) == max(sampled_day_data(1:i+ct,2)) ...
