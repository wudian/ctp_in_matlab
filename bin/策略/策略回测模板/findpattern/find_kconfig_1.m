function signal = find_kconfig_1(daydata, inpara)
%%% n连发
%%% INPUT
...
if freq ~= 1
for i = 1 : n_sample-ct 
    if sampled_day_data(i,3) <  sampled_day_data(i+1,3) && sampled_day_data(i,2) <  sampled_day_data(i+1,2)  %% ct连增
        l1 = 1;
        for j = i:i+ct-2
        ...
        if l1 && sampled_day_data(i+ct-1,2) == max(sampled_day_data(1:i+ct,2)) && sampled_day_data(i+ct-1,1) < sampled_day_data(i+ct-1,4) && sampled_day_data(i+ct,1) > sampled_day_data(i+ct,4)...
              && sampled_day_data(i+ct-1,2) - min(sampled_day_data(i:i+ct-1,3)) >= amp_down
...
%                 && sampled_day_data(i+ct,2) - min(sampled_day_data(i:i+ct-1,3)) <= amp_down              %% 如果l1 == 1说明找到了一个
    ...
    elseif sampled_day_data(i,2) >  sampled_day_data(i+1,2) && sampled_day_data(i,3) >  sampled_day_data(i+1,3)%% ct减
        l1 = 1;
        for j = i:i+ct-2
        ...
        if l1 && sampled_day_data(i+ct-1,3) == min(sampled_day_data(1:i+ct,3)) && sampled_day_data(i+ct-1,1)>sampled_day_data(i+ct-1,4) && sampled_day_data(i+ct,1)<sampled_day_data(i+ct,4)...
               &&   max(sampled_day_data(i:i+ct-1,2))-sampled_day_data(i+ct-1,3) >= amp_down 
%                && (max(sampled_day_data(i+ct,1),sampled_day_data(i+ct,4)) ==sampled_day_data(i+ct,2) || min(sampled_day_data(i+ct,1),sampled_day_data(i+ct,4)) ==sampled_day_data(i+ct,3))...
%                 &&   max(sampled_day_data(i:i+ct-1,2))-sampled_day_data(i+ct-1,3) <= amp_down  
%                 && sum(daydata(max(1,sum(sampled_day_data_size(1:i+ct,1))-vol_num+1):sum(sampled_day_data_size(1:i+ct,1)),5))/vol_num*vol_multiple > daydata(sum(sampled_day_data_size(1:i+ct,1)),5)...
...
t = 0; 
  for i = 1:k 
    if signal_in_sec(i) < n_sample && sum(sampled_day_data_size(1:signal_in_sec(i))) + 1 ~= inpara{2}(5)
        ...
        signal.break(t) = signal.inplace(t);
    end
  end %%  for i = 1:k 
  
  if t > 0
          signal.name = 'kconfig_1'; %% OUTPUT 1 
