function record = strategy_simple(todaydata, oper_infos, para)
                   ...
                   if oper_temp{j}.inplace(k) == i          %% ！！按时间顺序进行处理，避免按模型顺序查找，导致时间点偏后的交易先进场！！
                     ...
                     if oper_temp{j}.inplace(k) >= p_out    
                          ...
                          rec(r,:) = [oper_temp{j}.inplace(k), oper_temp{j}.inprice(k), oper_temp{j}.outplace(k), oper_temp{j}.outprice(k), ...
                                   direct, oper_temp{j}.profit(k),oper_temp{j}.bestprice(k), oper_temp{j}.badprice(k),oper_temp{j}.bestpricetime(k), oper_temp{j}.badpricetime(k)];   %% [inplace, inprice, outplace, outprice, direct, profit] 
