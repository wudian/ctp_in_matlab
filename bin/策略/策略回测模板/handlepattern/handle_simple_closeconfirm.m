function oper_info = handle_simple_closeconfirm(todaydata, signal, outpara)
...
for i = 1:length(signal.inplace)
    ...
    if signal.direct(i) > 0  
       ...
       for j = p_in : len_day-1
            ...
            if j==p_in && todaydata(p_in,4) < price_in
                oper_info.outplace(i) = j+1; 
                oper_info.outprice(i) = todaydata(j+1,3); 
                oper_info.profit(i)   = todaydata(j+1,3) - price_in;  
            ...
            elseif j ~= p_in && price_in >= todaydata(j,1) + s       %% 1 开盘跳空止损出场  (进场点不用考虑开盘跳空问题)
    ...
    elseif signal.direct(i)< 0 
       ...
       for j = p_in : len_day-1
            ...
            if  j==p_in && todaydata(j,4) >price_in
                oper_info.outplace(i) = j+1; 
                oper_info.outprice(i) = todaydata(j+1,2); 
            ...
            elseif j ~= p_in && todaydata(j,1) >= price_in + s       %% 1 开盘跳空止损出场   (进场点不用考虑开盘跳空问题)
                oper_info.outplace(i) = j; 
                oper_info.outprice(i) = todaydata(j,1); 
