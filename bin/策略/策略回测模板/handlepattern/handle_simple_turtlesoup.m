function oper_info = handle_simple_turtlesoup(todaydata, signal, outpara)
...
for i = 1:length(signal.inplace)
    ...
    s = min(s,abs(signal.stop(i)-price_in)+r);
...
%                if  todaydata(j+1,1) - price_in < e       %% 3.1  保单跳空 出场 
                oper_info.outplace(i) = j; 
                oper_info.outprice(i) = price_in+d; 
                oper_info.profit(i)   = d; 
                [z,x] = max(todaydata(p_in:len_day,2));
                oper_info.bestprice(i) = z;
                [h,g] = min(todaydata(p_in:len_day,3));
                oper_info.badprice(i) = h;
...
%                
       ...
       if j == len_day-1                                     %%  6 当日收盘出场 
                ...
                oper_info.outprice(i) = todaydata(len_day,3); 
                oper_info.profit(i)   = todaydata(len_day,3) - price_in; 
               [z,x] = max(todaydata(p_in:len_day,2));
                oper_info.bestprice(i) = z;
                [h,g] = min(todaydata(p_in:len_day,3));
                oper_info.badprice(i) = h;
...
%                end
       ...
       if  j == len_day-1                                   %%  6 当日收盘出场 
            oper_info.outplace(i) = len_day; 
            oper_info.outprice(i) = todaydata(len_day,2); 
            oper_info.profit(i)   =  price_in - todaydata(len_day,2); 
            [z,x] = min(todaydata(p_in:len_day,3));
                oper_info.bestprice(i) = z;
                [h,g] = max(todaydata(p_in:len_day,2));
                oper_info.badprice(i) = h;
