function oper_info = handle_dynamic(todaydata, signal, outpara)
...
for i = 1:length(signal.inplace)
    ...
    if signal.direct(i) > 0   
        for j = 1 : p_in-2
            if todaydata(p_in-j,2) < todaydata(p_in-j-1,2) && todaydata(p_in-j,3) < todaydata(p_in-j-1,3)
                loss = price_in - todaydata(p_in-j,3) + r;
            end
        end
        if loss ==0
            loss = price_in - min(todaydata(1:p_in,3)) + r;
        end
        s = min(loss,s);
    ...
    elseif signal.direct(i)< 0    
        for j = 1 : p_in-2
            if todaydata(p_in-j,2) > todaydata(p_in-j-1,2) && todaydata(p_in-j,3) > todaydata(p_in-j-1,3)
                loss = todaydata(p_in-j,2) - price_in + r;
            end
        end
        if loss ==0
            loss = max(todaydata(1:p_in,2))- price_in  + r;
        end
        s = min(loss,s);
