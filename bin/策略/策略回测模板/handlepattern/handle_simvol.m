function oper_info = handle_simvol(todaydata, signal, outpara)
...
%%  6 outpara =   [价格单位波r 止损s 保单触发d 保单e 第一档浮盈触发p 第一档回撤百分比q 第二档浮盈触发u 第二档回撤百分比v 出现山形次数n 中间成交量的下限m]
%%
%%% OUTPUT
...
for i = 1:length(signal.inplace)
    ...
    if signal.direct(i) > 0                
       for j = p_in : len_day-2
            ...
            elseif  price_in >= todaydata(j,3) + s   %% 2 止损出场 (开盘没有跳空，故止损价是拿得到的)
                oper_info.outplace(i) = j; 
             ...
             elseif  A<p && j ~= p_in && todaydata(j-1,2) <= todaydata(j,2) && todaydata(j+1,2)>=todaydata(j,2) ...
    ...
    elseif signal.direct(i)< 0      
       ...
       for j = p_in : len_day-2
            ...
            elseif  todaydata(j,2) >= price_in + s   %% 2 止损出场 
                ...
                oper_info.badprice(i) = h;
                break;
            elseif  A<p && j ~= p_in && todaydata(j-1,3) >= todaydata(j,3) && todaydata(j+1,3)<=todaydata(j,3) ...
                    && (j+1<=75|| 75<j-1<=133 || 135<j-1<173 || 175<j-1)...
                    && todaydata(j-1,5) < todaydata(j,5) && todaydata(j+1,5)>=todaydata(j,5) &&  todaydata(j,5) >=m    %% 3 寻找价格和成交量的山形
                num=num+1;
                if  num == n
                oper_info.outplace(i) = j + 2; 
                oper_info.outprice(i) = todaydata(j+2,1); 
                oper_info.profit(i)   =  price_in - todaydata(j+2,1); 
                [z,x] = min(todaydata(p_in:len_day,3));
                oper_info.bestprice(i) = z;
                [h,g] = max(todaydata(p_in:len_day,2));
                oper_info.badprice(i) = h;
